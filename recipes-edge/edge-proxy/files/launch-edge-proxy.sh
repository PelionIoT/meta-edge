...
EDGE_PROXY_URI_RELATIVE_PATH=$(jq -r .edge_proxy_uri_relative_path /edge/etc/edge-proxy.conf.json)
...
if [[ $EDGE_PROXY_DEBUG = "false" ]]; then
    echo "edge-proxy logging is disabled.  To see logs, run \"sed -i 's/EDGE_PROXY_DEBUG=false/EDGE_PROXY_DEBUG=true/g' /edge/system/bin/launch-edge-proxy.sh\" and restart edge-proxy"
    # this is known as bash exec redirection.
    # see https://www.tldp.org/LDP/abs/html/x17974.html
    exec >/dev/null 2>&1
fi

exec /edge/system/bin/edge-proxy \
